...
'''
    def get_state(self):
        """Load world state from a db"""
        try:
            cur.executemany(
                "INSERT INTO state.glob(key, data) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET data=excluded.data",
                self.glob.items(),
            )

            for region, region_data in self.glrg.items():
                cur.executemany(
                    "INSERT INTO state.glrg(region, key, data) VALUES(?, ?, ?) ON CONFLICT(region, key) DO UPDATE SET data=excluded.data",
                    region,
                    region_data.items(),
                )
    ...
    def save_state(self, state):
        try:
            ...
            for region, region_data in state.glrg.items():
                cur.executemany(
                    "INSERT INTO state.glrg(region, key, data) VALUES(?, ?, ?) ON CONFLICT(region, key) DO UPDATE SET data=excluded.data",
                    region,
                    region_data.items(),
    ...
    def save(self, database, table, key, value):
        self.connection.execute(
            "INSERT INTO $database.$table VALUES(:key, :value) ON CONFLICT(key) DO UPDATE SET data=excluded.data",
            {"database": database, "table": table, "key": key, "value": value},
        )
'''
